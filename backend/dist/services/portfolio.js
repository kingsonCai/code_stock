...
// 用户最近交易记录
const recentTradeRecords = new Map();
class PortfolioService {
    /**
     * 初始化用户持仓数据
     */
    initUserPortfolio(userId, positions) {
        ...
        logger_js_1.logger.info(`Initialized portfolio for user ${userId}`);
    }
    /**
     * 获取用户投资组合
     */
     ...
     * 根据市场数据更新所有用户的持仓
     */
    updateFromMarketData(marketPrices) {
        if (positionSnapshots.size === 0) {
            this.initUserPortfolio('default', this.getDefaultPositionSnapshots());
            logger_js_1.logger.info('Portfolio service: auto-initialized default user portfolio');
        }
        positionSnapshots.forEach((snapshots, userId) => {
            let changed = false;
            const updatedPositions = snapshots.map((snap) => {
            ...
            if (changed) {
                logger_js_1.logger.debug(`Portfolio updated for user ${userId}, broadcasting`);
     ...
     * 广播账户更新
     */
    broadcastAccountUpdate(userId, positionUpdater) {
        const snapshots = positionSnapshots.get(userId);
        if (!snapshots)
            return;
        ...
        if (server_js_1.wsServer) {
            ...
            server_js_1.wsServer.broadcast('account:default', portfolio);
        }
    }
    /**
     * 获取最近交易记录
     */
    getRecentTrades(userId, limit = 10) {
