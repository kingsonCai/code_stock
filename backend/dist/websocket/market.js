...
class MarketSimulator {
     ...
     */
    start() {
        logger_js_1.logger.info('Market simulator started');
        // 为每个股票启动价格更新
        ALL_STOCKS.forEach((stock) => {
        ...
        this.accountBroadcastInterval = setInterval(() => {
            ...
            portfolio_js_1.portfolioService.updateFromMarketData(prices);
        }, 5000);
    }
    /**
     * 停止模拟器
    ...
    stop() {
        this.intervals.forEach((interval, symbol) => {
            ...
            logger_js_1.logger.debug(`Stopped price updates for ${symbol}`);
        });
        ...
        logger_js_1.logger.info('Market simulator stopped');
    }
    /**
     * 启动单个股票的价格更新
     */
     ...
     * 生成历史K线数据
     */
    generateKlineData(symbol, timeframe, count = 100) {
